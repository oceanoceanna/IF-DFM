...
def alternate_run(params, wandb,logger):
    cvr_model = None
    sub_model = None
    dataset = get_criteo_dataset(params,logger)
    sub_params = deepcopy(params)

    sub_params["dataset"] = "fsiwsg_cd_"+str(params["CD"])+"_end_"+str(params["training_end_day"])+"_seed_"+str(params["seed"]) +"_test_duration_" +str(params['test_gap'])+"_train_duration_" +str(params['training_duration'])+ "_valid_" + str(params['valid_test_size'])
    np.random.seed(params["seed"])
    sub_dataset = get_criteo_dataset(sub_params,logger)["train"]
    np.random.seed(params["seed"])
    ...
    for i in range(2):
        ...
        sub_model = sub_train(cvr_model, sub_dataset, params,logger)
        seed_everything(params)
        cvr_model = cvr_train(sub_model, dataset, params, wandb,logger)
        params["idx"] += 1

def sub_train(cvr_model, sub_dataset, params,logger):
...
def cvr_train(sub_model, datasets, params, wandb,logger):
    ...
    return train(models, optimizers, data_loaders, params, wandb,logger)


def train(models, optimizers, data_loaders, params, wandb,logger):
    ...
    for ep in range(20):
        ...
        logger.info("Val ep{}, llloss {}".format(ep, vllloss))
        tauc, tprauc, tllloss = test(models["model"], test_data, params)
        logger.info("Test ep{}, auc {}, prauc {}, llloss {}".format(ep, tauc, tprauc, tllloss))
        ...
        if len(val_llloss) - val_llloss.index(min(val_llloss)) > params["early_stop"]:
            ...
            logger.info("Early stop at ep {}. Best ep {}. Best val_lloss {}.".format(ep, best_ep, min(val_llloss)))
            logger.info("Final test evaluation: auc {}, prauc {}, llloss {}.".format(test_auc[best_ep], test_prauc[best_ep], test_llloss[best_ep]))
            break
        train_loss = []
        for step, batch in enumerate(tqdm(train_data)):
            ...
            optimizer.step()
            params["log_step"] += 1
        logger.info("Train ep{}, loss {}".format(ep, np.mean(train_loss)))
