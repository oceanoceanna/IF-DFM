...
def cross_entropy_loss_sum(targets, outputs, params=None):
    z = targets["label"]
    x = outputs["logits"]
    x = torch.reshape(x, (-1,))
    z = z.float()
    loss = torch.nn.functional.binary_cross_entropy_with_logits(x, z, reduction='sum')
    return {"loss": loss}
