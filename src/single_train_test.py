...
def train(models, optimizers, data_loaders, params, wandb,logger):
    ...
    logger.info("train_data_size: {}".format(S))
    ...
    for ep in range(20):
        ...
        logger.info("Test ep{}, auc {}, prauc {}, llloss {}, val_loss {}".format(ep, tauc, tprauc, tllloss,vllloss))
        ...
        save_path = "./seed_train/"+ str(params['base_model'])+'/' + str(params['base_model']) + '_model_end' + str(params['training_end_day'])+"_duration_"+str(params['training_duration'])+"_test_duration_"+str(params['test_gap'])+"_method_"+str(params['method'])+ "_valid_" + str(params["valid_test_size"])+"_seed_" + str(params["seed"])+".pth"
        torch.save(models["model"].state_dict(), save_path)
        
        if len(val_llloss) - val_llloss.index(min(val_llloss)) > 6:
        ...
        for step, batch in enumerate(tqdm(train_data)):
            ...
            optimizer.step()
            log_step += 1
        logger.info("Train ep{}, training loss {}".format(ep, np.mean(train_loss)))

def train_old(models, optimizers, data_loaders, params, wandb):
    train_data = data_loaders["train_data"]
    S = len(train_data.dataset)
    ...
    for ep in range(params["train_epoch"]):
        ...
        if params["method"] not in ["nnDF"]:
            ...
            for step, batch in enumerate(tqdm(train_data)):
                ...
                optimizer.step()
                log_step += 1
            print("Train ep{}, training loss {}".format(ep, np.mean(train_loss)))
        else:
            train_loss = []
            all_jd_pos = [0,0]
            all_jd_neg = [0,0]
...
def next_run(params, wandb,logger):
    ...
    train(models, optimizers, data_loaders, params, wandb,logger)
